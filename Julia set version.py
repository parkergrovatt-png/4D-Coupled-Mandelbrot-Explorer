...
def compute_frame(width, height, cx, cy, scale, k_real, k_imag, max_iter, escape_radius):
    ...
    for j in prange(height):
        ...
        for i in range(width):
            ...
            # initial c0 is the pixel coordinate
            c = complex(x0, y0)
            z = complex(0.0, 0.0)
            c_iter = c
            k = complex(k_real, k_imag)
            ...
            for n in range(max_iter):
                ...
                z = z * z + c_iter
                # c_{n+1} = c_n^2 + k
                c_iter = c_iter * c_iter + k

                # check escape using z primarily, but also c_iter (coupling may blow either)
                if (z.real * z.real + z.imag * z.imag) > (escape_radius * escape_radius):
                    escaped = True
                    break
                if (c_iter.real * c_iter.real + c_iter.imag * c_iter.imag) > (escape_radius * escape_radius):
                    escaped = True
                    break
...
def main():
    ...
    while running:
        ...
        # compute frame
        img = compute_frame(WIDTH, HEIGHT, center_x, center_y, scale, k_real, k_imag_t, MAX_ITER, ESCAPE_RADIUS)
